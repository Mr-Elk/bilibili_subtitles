...
完整提取成功后会替换 manifest、索引和严格命名的旧字幕，同时保留同目录里的学习笔记及其他非受管文件；单 P 提取则原子更新该 P，并保留其他分 P 和用户文件。如果提取期间用户文件发生变化，发布会停止并保留用户的当前版本，不会用 staging 中的旧副本覆盖。旧版目录第一次执行单 P 更新时，会自动从已有 Markdown 迁移出清单。任何中途失败都会保留上一次成功结果，同一 BV 的并发提取会被拒绝。
